...
void Server::handleVoteReady(Client &client) {
  bool status = write_uint32(client.fd, NetworkEvents::VoteReady);
  if (!status) {
    TraceLog(LOG_WARNING,
             "Couldn't write GetRoomList NetworkEvent to client_id=%ld,fd=%d",
             client.client_id, client.fd);
  ...
  status = write_uint32(client.fd, gm.GetReadyPlayers());
  if (!status) {
    TraceLog(
        LOG_WARNING,
...
void Server::serverSetEvent(Client &client, NetworkEvents event) {
  bool status = write_uint32(client.fd, event);
  if (!status) {
    TraceLog(
        LOG_WARNING, "Couldn't write %s NetworkEvent to client_id=%ld,fd=%d",
        network_event_to_string(event).c_str(), client.client_id, client.fd);
    client_error(client);
  }
}
...
void Server::handleShootBullet(Client &client) {
  ...
  status = write_uint32(client.fd, (uint32_t)status);
  if (!status) {
    TraceLog(LOG_WARNING,
...
void Server::handleJoinRoom(Client &client) {
  bool status;
  uint32_t room_id;
  status = read_uint32(client.fd, room_id);
  if (!status) {
    TraceLog(LOG_WARNING,
  ...
  status = write_uint32(client.fd, (uint32_t)status * room_id);
  if (!status) {
...
void Server::handleLeaveRoom(Client &client) {
  ...
  try {
    ...
    TraceLog(LOG_WARNING,
             ...
             "client_id=%ld,fd=%d",
             client.room_id, client.client_id, client.fd);
    client_error(client);
  }
...
void Server::handleUpdateGameState(Client &client) {
  try {
    ...
    bool status = write_json(client.fd, gameState);
    if (!status) {
      TraceLog(LOG_WARNING,
...
void Server::handleUpdateRoomState(Client &client) {
  ...
  try {
    ...
    status = write_json(client.fd, room_json);
    if (!status) {
    ...
    TraceLog(LOG_WARNING, "Invalid room id %lu from client_id=%ld,fd=%d",
             room_id, client.client_id, client.fd);
    client_error(client);
    return;
  }
}
...
void Server::handleUpdatePlayers(Client &client) {
  try {
    ...
    status = write_json(client.fd, players_json);
    if (!status) {
      TraceLog(LOG_WARNING,
...
void Server::handleUpdateAsteroids(Client &client) {
  try {
    ...
    status = write_json(client.fd, asteroids_json);
    if (!status) {
      TraceLog(LOG_WARNING,
...
void Server::handleUpdateBullets(Client &client) {
  try {
    ...
    status = write_json(client.fd, bullets_json);
    if (!status) {
      TraceLog(LOG_WARNING,
